pipeline {
  ...
  environment {
    ...
    GITHUB_REPO = "ergon-automation-labs/ergon-mcp"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/mcp_bot/bin/mcp_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyMcp.Release.create()'"
          $RELEASE_BIN eval 'BotArmyMcp.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyMcp.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyMcp.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_mcp/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_mcp/releases/start_erl.data)
        fi
