# Getting Started with bot_army_mcp

This guide walks you through setting up MCP Bot for local development.
...
DATABASE_NAME=bot_army_mcp_dev
```
...
MCP Bot uses the **BotArmy.GenBot** harness to run skills. Skills are discrete, reusable units of work triggered via NATS messages.

### Creating Your First Skill

1. **Start with the template** — `lib/bot_army_mcp/skills/example.ex` is a fully documented template
2. **Copy and rename** it for your skill:
   ```bash
   cp lib/bot_army_mcp/skills/example.ex lib/bot_army_mcp/skills/my_skill.ex
   ```
...
```elixir
defmodule BotArmyMcp.Skills.Classify do
  use BotArmy.Skill

  def name, do: :classify
  def description, do: "Classifies text into categories"
  def nats_triggers, do: ["bot.bot_army_mcp.command.classify"]
...
```elixir
# In lib/bot_army_mcp/application.ex or similar

defmodule BotArmyMcp.MyBot do
  use BotArmy.GenBot,
    skills: [
      BotArmyMcp.Skills.Classify,
      BotArmyMcp.Skills.ExtractEntities
    ],
    bot_id: :my_bot
end
```
...
```bash
# Make your changes
edit lib/bot_army_mcp/skills/my_skill.ex
...
This creates a release named `mcp_bot` that is deployed to `/opt/ergon/releases/mcp_bot/` on the server.
...
3. Extracts and deploys to `/opt/ergon/releases/mcp_bot/`
